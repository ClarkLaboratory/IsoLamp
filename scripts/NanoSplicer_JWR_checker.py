'''
    This script is modified from https://github.com/shimlab/NanoSplicer/blob/master/bin/JWR_checker.py

    Modification:
       * Only output a CSV file (no hdf, also this removes the need to install hdf in the conda env)
       * integrate helper.py function in NanoSplicerso I don't need two scripts.
       * optionally take a minimum JAQ score as a threshold, and output the reads which always have a JAQ above this threshold?
    ...
    Output:
        A CSV file with given output_name will be generated with the following 
        columns:
...
class JWR_checker_param:
    def __init__(self, arg = sys.argv):
        ...
        for opt, arg in opts:
            if opt in ("-h", "--help"):
                ...
                if len(self.g_loc) != 2:
                    err_msg("ERROR:Invalid genome-loc.")
                    sys.exit(1)
            elif opt == "--threads":
    ...
    def print_help(self):
        ...
        Usage: python3 {} [OPTIONS] <BAM file> <output csv file>
        ...
        Options:
            ...
            --min-jaq        Only output JWRs with JAQ>=min-jaq. <default: 0>
        '''.format(sys.argv[0])
...
def main():
    ...
    for read in algn_file.fetch():
        if 'M' in read.cigarstring:
            warning_text =\
                ...
                '''
            warning_msg(textwrap.dedent(warning_text))
    ...
    # filter by JAQ
    if param.min_jaq:
        d = d[d['JAQ'] >= param.min_jaq]
    
    d.to_csv(param.outfile)
